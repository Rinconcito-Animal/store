import{i as A,o as k,a as f,e as q,f as E,d as g,r as C,h as N,u as P,j as T,k as b,q as z,c as F,w as j,g as R,l as _}from"./navbar-BaqoyTjM.js";A();A();const $=document.querySelectorAll(".account-nav-btn"),G=document.querySelectorAll(".account-section"),H=document.getElementById("profile-data"),v=document.getElementById("receipts-list"),L=document.getElementById("pets-grid"),d=document.getElementById("pet-modal"),u=document.getElementById("profile-modal"),h=document.getElementById("add-pet-form"),I=document.getElementById("edit-profile-form"),w=document.getElementById("open-pet-modal"),M=document.getElementById("close-pet-modal"),D=document.getElementById("open-profile-modal"),x=document.getElementById("close-profile-modal");let r=null;$.forEach(t=>{t.addEventListener("click",()=>{const e=t.getAttribute("data-target");$.forEach(a=>a.classList.remove("active")),t.classList.add("active"),G.forEach(a=>a.classList.remove("active")),document.getElementById(e).classList.add("active")})});const i=(t,e)=>{e?t.classList.add("active"):t.classList.remove("active")};w&&w.addEventListener("click",()=>i(d,!0));M&&M.addEventListener("click",()=>i(d,!1));d&&d.addEventListener("click",t=>{t.target===d&&i(d,!1)});D&&D.addEventListener("click",()=>{r&&(document.getElementById("user-name").value=r.fullName||"",document.getElementById("user-email").value=r.email||"",document.getElementById("user-phone").value=r.phone||"",document.getElementById("user-address").value=r.address||""),i(u,!0)});x&&x.addEventListener("click",()=>i(u,!1));u&&u.addEventListener("click",t=>{t.target===u&&i(u,!1)});k(f,async t=>{t?(console.log("Usuario detectado:",t.uid),await B(t.uid)):(console.log("No hay usuario logueado, redirigiendo..."),window.location.href="/signup.html")});async function B(t){try{const e=await q(E(g,"users",t));e.exists()?(r=e.data(),Q(r),J(r.pets||[]),await O()):(console.error("No se encontró el perfil en Firestore"),H.innerHTML="<p>Error al cargar el perfil. Por favor intenta de nuevo.</p>")}catch(e){console.error("Error cargando datos:",e)}}function Q(t){H.innerHTML=`
